...
for i in $(seq 0 $participants); do
  docker run  -v $(pwd)/test:/test -e DD_REPORT_PATH="/test/dependency-check-report-5.xml" \
    -v $(pwd)/tmp:/code \
  ...
  docker run  -v $(pwd)/test:/test -e DD_REPORT_PATH="/test/findings-0.csv" \
    -v $(pwd)/tmp:/code \
    -e DD_PRODUCT_TAGS="cluster/production" -e DD_PRODUCT_TAGS="cluster/production" -e  DD_TEAM="fellowship-of-the-ring-$i" \
    -e IS_CREATE_GROUPS="true" \
